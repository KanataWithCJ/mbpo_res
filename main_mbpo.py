...
def init_log(main_path, verbose):
    def _init_logging(verbose, log_path):
    ...
    log_path = os.path.join(main_path, 'logModel.txt')
    _init_logging(verbose=verbose, log_path=log_path)
    logging.info("Results stored in {main_path}")

def train(args, env_sampler, predict_env,predict_bias, agent,residual_agent, env_pool, model_pool):
    total_step = 0
    reward_sum = 0
    rollout_length = 1
...
# train_predict_model(args, env_pool, predict_env)
    ...
    for epoch_step in range(args.num_epoch):
        ...
        with tqdm(total=args.epoch_length) as pbar:
            pbar.set_description("Epoch %d" % epoch_step)
            for i in count():
                ...
                if cur_step > 0 and cur_step % args.model_train_freq == 0 and args.real_ratio < 1.0:
                    ...
                    rollout_model(args, predict_env, agent, residual_agent,model_pool, env_pool, rollout_length)
...
def exploration_before_start(args, env_sampler, env_pool, agent):
    with tqdm(total=args.init_exploration_steps) as pbar:
        pbar.set_description("exploration before start")
...
def train_predict_model(args, env_pool, predict_env):
    ...
    # print("reward.shape:%s" % str(reward.shape))
    # print("state.shape:%s" % str(delta_state.shape))
    predict_env.model.train(inputs, labels, batch_size=256, holdout_ratio=0.2)

def train_bias_model(args, env_pool,predict_bias,predict_env):
    # Get all samples from environment
    state, action, reward, next_state, done = env_pool.sample(len(env_pool))

    # print(reward.shape)
    predict_next_states, predict_rewards, predict_terminals, info = predict_env.step(state, action)
    
    bias_delta_state = next_state-predict_next_states-state
    bias_reward = reward-predict_rewards.squeeze()
    # print("bias_reward.shape:%s" % str(bias_reward.shape))
    # print("bias_delta_state.shape:%s" % str(bias_delta_state.shape))
    inputs = np.concatenate((state, action), axis=-1)
    labels = np.concatenate((np.reshape(bias_reward, (bias_reward.shape[0], -1)), bias_delta_state), axis=-1)

    predict_bias.model.train(inputs, labels, batch_size=256, holdout_ratio=0.2)
...
def main(args=None):
    ...
    residual_agent = SAC(env.observation_space.shape[0],env.action_space,args)
    ...
    if args.model_type == 'pytorch':
        ...
        bias_model = EnsembleDynamicsModel(args,args.num_networks, args.num_elites, state_size, action_size, args.reward_size, args.pred_hidden_size,
                                          use_decay=args.use_decay)
    else:
        ...
        bias_model = construct_model(obs_dim=state_size, act_dim=action_size, hidden_dim=args.pred_hidden_size, num_networks=args.num_networks,
                                    num_elites=args.num_elites)

    # Predict environments
    predict_env = PredictEnv(env_model, args.env_name, args.model_type)
    predict_bias = PredictEnv(env_model, args.env_name, args.model_type)
    ...
    train(args, env_sampler, predict_env,predict_bias, agent, residual_agent,env_pool, model_pool)
